...
- Let editor code keep using the normal typed `BlockClient` and `BlockHandle` APIs. A child-process client runs the ordinary client over a tunnel through the parent's connection instead of opening another server connection.
...
The parent owns the server websocket. Each plugin instance constructs a `BlockClient` whose frames travel over the plugin IPC connection instead of a socket of its own; the parent forwards them, unread, over its connection. One connection carries any number of separate clients, each tagged by id and given its own watches, presence and sequencing by the server, so the child's client behaves exactly like one that dialled in itself. From editor code, `get_block::<B>`, `BlockHandle::read`, `operate`, undo/redo, creation, relationships, references, and presence continue to work as they do in process.

This first implementation does not sandbox plugins or assign per-plugin capability scopes. A child executable is already arbitrary code running as the user, and its tunnelled client may use the same workspace operations available to the parent client. Normal account, workspace, and block permissions still apply at the existing client/server boundary, but the parent bridge is not a new security boundary. Sandboxing, permission prompts, and restricted clients are explicitly future work and must not complicate the MVP protocol.

Refactor `block-client` so its worker can reach the server over either its own websocket or a tunnel through another client's connection. The parent keeps sole ownership of the websocket; everything above it, including operation sequencing and history, runs unchanged in the child. The tunnel carries the ordinary block protocol rather than a second semantic protocol, so the child reuses typed blocks without the parent having to understand what it is asking for.
...
Keep two logical message families on the framed plugin connection. The editor protocol controls plugin instances, render regions, input, and app-level actions. The tunnel protocol carries block-protocol frames for an instance's `BlockClient` and is independent of editor UI lifecycle.

Add bounded editor messages for:

- editor discovery and registration metadata, checked against the package manifest;
- opening an instance with its block ID, block type, editor role, and a handle to the runtime's tunnelled client;
...
The tunnel protocol carries the block protocol itself, so fetching and watching blocks, creating them, submitting typed serialized operations, history, properties, reference lists, relationships, dynamic artifacts, and presence all work because the child is running the same client the parent is. Frames are forwarded in order and answered by the server, not the parent.

The UI protocol must expose capabilities rather than assume every editor implements every feature. Unsupported optional regions and actions retain the defaults currently provided by `BlockEditor`. Protocol limits should be defined per payload class: tunnelled block data, media-backed blocks, operation batches, presence, and surface descriptors cannot all share the demo's current one-megabyte frame limit.
...
- a new `block-editor-plugin` guest SDK: instance routing, tunnelled-client setup, host UI requests, `egui` input adaptation, region rendering, and target entry points;
...
First-party editor packages depend on the guest SDK and `block-client`, but not on `block-app`. They open their typed handles from the tunnelled client and retain the same read/operate/subscription patterns used today. The plugin build of `block-client` must exclude direct websocket, account-management, and platform transport dependencies that a plugin does not use, keeping plugin artifacts smaller without creating a second client API. Editor-only reusable cores such as `logicgame`, canvas geometry, database layouts, and renderers move with or below their plugin packages rather than remaining reachable through `block-app`.

Split the current text editor into two reusable layers before migrating it. `text-editor-core` remains the UI-independent editing engine for diffing, syntax highlighting, and cursor data. A new `text-editor-view` package owns the editor/view behavior currently in `block-app`, including `egui` interaction, layout, font handling, embeds, selection, cursor presentation, and the adapter that drives `text-editor-core`. The text plugin depends on `text-editor-view` and supplies its tunnelled `BlockHandle<Text>` and presence access; neither reusable text package depends on `block-app` or plugin transport.

The guest SDK supplies the tunnelled `BlockClient`, local UI state, named-region callbacks, and declarative editor capabilities. Block state, operations, references, history, and presence use the real `block-client` API rather than SDK-specific imitations. Provide a package template, manifest validation command, build command, and a revised editor guide based on `counter`.
...
The first milestone is deliberately a narrow vertical slice: a real synchronized counter block opened through a tunnelled `BlockClient` and rendered by a plugin in an ordinary tab. It uses the existing WebExternalImage and Windows DXGI work, supports one plugin instance and one main region, and omits creation UI, previews, presence, references, and third-party discovery. That proves the tunnelled client and texture path before generalizing either one.
...
3. **Define the tunnel protocol.** Add bounded request and response messages that carry block-protocol frames for one instance over the existing framed IPC transport.
4. **Implement the parent client bridge.** Open a tunnel per instance on the parent's client, forward its frames to the server, and return the server's replies in order without opening another connection.
5. **Implement the child tunnelled client.** Construct a `BlockClient` whose transport is the IPC tunnel, and preserve the existing optimistic operation and history behavior.
...
9. **Extract the minimum guest runtime.** Move handshake, tunnelled-client setup, single-instance routing, `egui` input, and frame production into `block-editor-plugin`.
...
12. **Support multiple instances per runtime.** Route instance-scoped UI messages, give each instance its own tunnelled client, stop a runtime after its last instance, and reject stale instance IDs.
13. **Complete tunnelled block watching.** Support concurrent typed handles, initial fetches, watch lifetimes, confirmed updates, disconnection, and resubscription after runtime restart.
14. **Complete tunnelled operations and history.** Cover pending operation ordering, acknowledgements, remote operations, history grouping, undo, redo, and authoritative replacement using existing client semantics.
15. **Tunnel block creation and properties.** Support typed creation, duplication, replacement, naming, parent changes, access reads, and dynamic-artifact descriptors through the child client.
16. **Tunnel references and presence.** Support reference-list watches, cached reference metadata, relationship helpers, presence reads/writes, and cleanup when a tunnelled client closes.
17. **Add tunnelled-client conformance tests.** Run the existing client behavior suite against direct and tunnelled clients and add transport interruption and reconnection cases.
18. **Add package failure recovery.** Distinguish missing, incompatible, crashed, timed-out, and surface-failed packages; restart first-party runtimes and reopen their tunnels.
...
27. **Add editor creation flows.** Extend manifests and the UI protocol for immediate and configured creation while editor code creates blocks through its tunnelled client.
...
29. **Add child and artifact UI actions.** Route child placement plus dynamic-artifact settings and regeneration UI between plugin regions and host chrome; perform block changes through tunnelled handles.
30. **Complete the guest authoring API.** Expose tunnelled-client access, declarative regions, theme/font-scale data, host UI actions, and a headless conformance harness.
...
38. **Migrate image, audio, and browser tab.** Use tunnelled handles plus the host platform integrations for imports, playback, and native webviews.
39. **Migrate video.** Package its timeline, player, and effects using tunnelled handles and host media playback while preserving frame-accurate operations.
...
41. **Migrate map and pixel ray tracer.** Package their rendering cores and UI using tunnelled handles and the new integrations, including previews and intrinsic sizing.
...
44. **Migrate logic game and version control.** Package challenge, quiz, version-control data, and worktree UI using the tunnelled client and ordinary child-process OS access.
...
50. **Migrate text.** Build the text plugin around `text-editor-view` and a tunnelled `BlockHandle<Text>`, including cursor presence, embeds, and intrinsic sizing.
...
52. **Migrate logic grid.** Package direct editing, GPU rendering, presence, challenge playback, nested components, and compiled-logic artifacts using tunnelled handles.
