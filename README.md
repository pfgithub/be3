...
```sh
./scripts/build-plugin-demo.sh --target x86_64-unknown-linux-gnu --profile release --app-executable target/x86_64-unknown-linux-gnu/release/block-app
./scripts/build-plugin-demo.sh --target x86_64-pc-windows-msvc --profile release --app-executable target/x86_64-pc-windows-msvc/release/block-app.exe
./scripts/build-plugin-demo.sh --target aarch64-apple-darwin --profile release --app-executable target/aarch64-apple-darwin/release/block-app --sign-identity 'Developer ID Application: Example'
```

The default output is `target/plugin-demo/<target>/<profile>`. Linux stages
`bin` and `libexec/be3`, Windows uses a private `plugin-demo` directory beside
