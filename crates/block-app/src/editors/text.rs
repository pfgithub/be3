...
enum PaletteAction {
    ...
    InsertBlock,
    OpenDialog(PaletteDialogKind),
}

/// Tags a variant of [`EditorCommand`] whose arguments (text, a byte offset,
/// or an enum of options) are collected in a follow-up [`PaletteDialog`]
/// rather than run immediately, unlike the argument-free/small-enum
/// variants [`palette_entries`] dispatches directly.
#[derive(Clone, Copy)]
enum PaletteDialogKind {
    MoveCursorLeftRight,
    Delete,
    MoveCursorUpDown,
    InsertText,
...
impl PaletteDialog {
    fn title(&self) -> &'static str {
        match self {
            Self::MoveCursorLeftRight { .. } => "Move Cursor",
            Self::Delete { .. } => "Delete",
            Self::MoveCursorUpDown { .. } => "Move Cursor Up/Down",
            Self::InsertText { .. } => "Insert Text",
            Self::Paste { .. } => "Paste Text",
            Self::SetSelection { .. } => "Set Selection",
            Self::ReplaceBlockReference { .. } => "Replace Block Reference",
            Self::Click { .. } => "Click",
            Self::Drag { .. } => "Drag",
            Self::DragSelectionHandle { .. } => "Drag Selection Handle",
            Self::ReplaceWholeFile { .. } => "Replace Whole File",
            Self::ToggleCollapseAt { .. } => "Toggle Collapse At",
        }
    }
}

const LR_DIRECTIONS: [LRDirection; 2] = [LRDirection::Left, LRDirection::Right];
const UD_DIRECTIONS: [UDDirection; 2] = [UDDirection::Up, UDDirection::Down];
const MOVE_MODES: [MoveMode; 2] = [MoveMode::Move, MoveMode::Select];
const VERTICAL_MOVE_MODES: [VerticalMoveMode; 3] = [
    VerticalMoveMode::Move,
    VerticalMoveMode::Select,
    VerticalMoveMode::Duplicate,
];
const CURSOR_STOPS: [CursorLeftRightStop; 7] = [
    CursorLeftRightStop::Byte,
    CursorLeftRightStop::Codepoint,
    CursorLeftRightStop::UnicodeGraphemeCluster,
    CursorLeftRightStop::Word,
    CursorLeftRightStop::UnicodeWord,
    CursorLeftRightStop::Line,
    CursorLeftRightStop::VisualLine,
];
const HORIZONTAL_METRICS: [CursorHorizontalPositionMetric; 4] = [
    CursorHorizontalPositionMetric::Byte,
    CursorHorizontalPositionMetric::Codepoint,
    CursorHorizontalPositionMetric::UnicodeGraphemeCluster,
    CursorHorizontalPositionMetric::Screen,
];
...
fn byte_offset_field(ui: &mut egui::Ui, label: &str, value: &mut usize, max: usize) {
    ui.horizontal(|ui| {
        ui.label(label);
...
fn palette_entries(language: TextLanguage) -> Vec<PaletteEntry> {
    let mut entries = vec![
        ...
        PaletteEntry {
            ...
            action: PaletteAction::Command(EditorCommand::Newline),
        },
        PaletteEntry {
            label: "Move Cursor...",
            action: PaletteAction::OpenDialog(PaletteDialogKind::MoveCursorLeftRight),
        },
        PaletteEntry {
            label: "Move Cursor Up/Down...",
            action: PaletteAction::OpenDialog(PaletteDialogKind::MoveCursorUpDown),
        },
        PaletteEntry {
            label: "Delete...",
            action: PaletteAction::OpenDialog(PaletteDialogKind::Delete),
        },
        PaletteEntry {
            label: "Insert Text...",
            action: PaletteAction::OpenDialog(PaletteDialogKind::InsertText),
        },
        PaletteEntry {
            label: "Paste Text...",
            action: PaletteAction::OpenDialog(PaletteDialogKind::Paste),
        },
        PaletteEntry {
            label: "Set Selection...",
            action: PaletteAction::OpenDialog(PaletteDialogKind::SetSelection),
        },
        PaletteEntry {
            label: "Replace Block Reference...",
            action: PaletteAction::OpenDialog(PaletteDialogKind::ReplaceBlockReference),
        },
        PaletteEntry {
            label: "Click...",
            action: PaletteAction::OpenDialog(PaletteDialogKind::Click),
        },
        PaletteEntry {
            label: "Drag...",
            action: PaletteAction::OpenDialog(PaletteDialogKind::Drag),
        },
        PaletteEntry {
            label: "Drag Selection Handle...",
            action: PaletteAction::OpenDialog(PaletteDialogKind::DragSelectionHandle),
        },
        PaletteEntry {
            label: "Replace Whole File...",
            action: PaletteAction::OpenDialog(PaletteDialogKind::ReplaceWholeFile),
        },
        PaletteEntry {
            label: "Toggle Collapse At...",
            action: PaletteAction::OpenDialog(PaletteDialogKind::ToggleCollapseAt),
        },
    ];
...
impl TextEditor {
    ...
    fn run_palette_action(&mut self, action: PaletteAction) {
        match action {
            ...
            PaletteAction::OpenDialog(kind) => self.open_dialog(kind),
        }
    }
    ...
    /// Builds the default field values for a [`PaletteDialog`], prefilled
    /// from the current cursor so running it unmodified is a no-op-ish
    /// identity action rather than jumping to byte 0.
    fn open_dialog(&mut self, kind: PaletteDialogKind) {
        let current = self.current_byte_offset();
        self.dialog = Some(match kind {
            PaletteDialogKind::MoveCursorLeftRight => PaletteDialog::MoveCursorLeftRight {
                mode: MoveMode::Move,
                direction: LRDirection::Right,
                stop: CursorLeftRightStop::UnicodeGraphemeCluster,
            },
            PaletteDialogKind::Delete => PaletteDialog::Delete {
                direction: LRDirection::Right,
                stop: CursorLeftRightStop::UnicodeGraphemeCluster,
            },
            PaletteDialogKind::MoveCursorUpDown => PaletteDialog::MoveCursorUpDown {
                direction: UDDirection::Down,
                mode: VerticalMoveMode::Move,
                metric: CursorHorizontalPositionMetric::Byte,
                stop: CursorLeftRightStop::UnicodeGraphemeCluster,
            },
            PaletteDialogKind::InsertText => PaletteDialog::InsertText {
                text: String::new(),
            },
            PaletteDialogKind::Paste => PaletteDialog::Paste {
                text: String::new(),
            },
            PaletteDialogKind::SetSelection => PaletteDialog::SetSelection {
                anchor: current,
                focus: current,
            },
            PaletteDialogKind::ReplaceBlockReference => PaletteDialog::ReplaceBlockReference {
                old: String::new(),
                new: String::new(),
            },
            PaletteDialogKind::Click => PaletteDialog::Click {
                offset: current,
                stop: CursorLeftRightStop::UnicodeGraphemeCluster,
                select: false,
                extend: false,
            ...
            PaletteDialogKind::Drag => PaletteDialog::Drag { offset: current },
            PaletteDialogKind::DragSelectionHandle => PaletteDialog::DragSelectionHandle {
                fixed: current,
                position: current,
            },
            PaletteDialogKind::ReplaceWholeFile => PaletteDialog::ReplaceWholeFile {
                text: self
                    .block
                    .read()
            ...
            PaletteDialogKind::ToggleCollapseAt => {
                PaletteDialog::ToggleCollapseAt { offset: current }
            }
        });
    }
    ...
    fn run_dialog(&mut self) {
        ...
        match dialog {
            PaletteDialog::MoveCursorLeftRight {
                mode,
                direction,
                stop,
            } => self
            ...
            PaletteDialog::Delete { direction, stop } => self
                .core
                .execute_command(EditorCommand::Delete { direction, stop }),
            PaletteDialog::MoveCursorUpDown {
                direction,
                mode,
                metric,
                stop,
            ...
            PaletteDialog::InsertText { text } => self
                .core
                .execute_command(EditorCommand::InsertText(text.as_bytes())),
            PaletteDialog::Paste { text } => self
                .core
                .execute_command(EditorCommand::Paste(text.as_bytes())),
            PaletteDialog::SetSelection { anchor, focus } => {
                let anchor = self.core.position(anchor);
                let focus = self.core.position(focus);
                self.core
                    .execute_command(EditorCommand::SetSelection { anchor, focus });
            }
            PaletteDialog::ReplaceBlockReference { old, new } => {
                if let (Ok(old), Ok(new)) =
                    (Uuid::parse_str(old.trim()), Uuid::parse_str(new.trim()))
                {
                    self.core
            ...
            PaletteDialog::Click {
                ...
                let position = self.core.position(offset);
                self.core.execute_command(EditorCommand::Click {
                    position,
                    mode: if select {
                        DragSelectionMode::select(stop)
                    } else {
                        DragSelectionMode::move_to(stop)
                    },
                    extend,
                    select_syntax_node,
                });
            }
            PaletteDialog::Drag { offset } => {
                let position = self.core.position(offset);
                self.core.execute_command(EditorCommand::Drag(position));
            }
            PaletteDialog::DragSelectionHandle { fixed, position } => {
                let fixed = self.core.position(fixed);
                let position = self.core.position(position);
                self.core
                    .execute_command(EditorCommand::DragSelectionHandle { fixed, position });
            }
            PaletteDialog::ReplaceWholeFile { text } => self
                .core
                .execute_command(EditorCommand::ReplaceWholeFile(text.as_bytes())),
            PaletteDialog::ToggleCollapseAt { offset } => {
                let position = self.core.position(offset);
                self.core
                    .execute_command(EditorCommand::ToggleCollapseAt(position));
            }
        }
    }

    /// Renders the follow-up options dialog opened by a [`PaletteAction::OpenDialog`]
    /// entry, for the [`EditorCommand`] variants that need arguments the
    /// palette's search list can't collect on its own.
    fn command_palette_dialog(&mut self, ctx: &egui::Context) {
        let Some(dialog) = self.dialog.as_mut() else {
            return;
        };
        let len = self.block.read().map(|d| d.bytes().len()).unwrap_or(0);
        let title = dialog.title();
        let valid = !matches!(
            dialog,
            PaletteDialog::ReplaceBlockReference { old, new }
                if Uuid::parse_str(old.trim()).is_err() || Uuid::parse_str(new.trim()).is_err()
        );
        let mut run = false;
        let mut cancel = false;
        let response = egui::Modal::new(egui::Id::new((
            "text-editor-command-dialog",
            self.block.id(),
        )))
        .show(ctx, |ui| {
            ui.set_width(360.0);
            ...
            match dialog {
                PaletteDialog::MoveCursorLeftRight {
                    ...
                    enum_combo(ui, "dlg-mode", mode, &MOVE_MODES);
                    enum_combo(ui, "dlg-direction", direction, &LR_DIRECTIONS);
                    enum_combo(ui, "dlg-stop", stop, &CURSOR_STOPS);
                }
                PaletteDialog::Delete { direction, stop } => {
                    enum_combo(ui, "dlg-direction", direction, &LR_DIRECTIONS);
                    enum_combo(ui, "dlg-stop", stop, &CURSOR_STOPS);
                }
                PaletteDialog::MoveCursorUpDown {
                    ...
                    enum_combo(ui, "dlg-direction", direction, &UD_DIRECTIONS);
                    enum_combo(ui, "dlg-mode", mode, &VERTICAL_MOVE_MODES);
                    enum_combo(ui, "dlg-metric", metric, &HORIZONTAL_METRICS);
                    enum_combo(ui, "dlg-stop", stop, &CURSOR_STOPS);
                }
                PaletteDialog::InsertText { text } | PaletteDialog::Paste { text } => {
                    ui.add(egui::TextEdit::multiline(text).desired_rows(3));
                }
                PaletteDialog::SetSelection { anchor, focus } => {
                    byte_offset_field(ui, "Anchor offset", anchor, len);
                    byte_offset_field(ui, "Focus offset", focus, len);
                }
                PaletteDialog::ReplaceBlockReference { old, new } => {
                    ui.horizontal(|ui| {
                        ui.label("Old block UUID");
                        ui.text_edit_singleline(old);
                    });
                    ui.horizontal(|ui| {
                        ui.label("New block UUID");
                        ui.text_edit_singleline(new);
                    });
                    if Uuid::parse_str(old.trim()).is_err() || Uuid::parse_str(new.trim()).is_err()
                    {
                        ui.colored_label(ui.visuals().error_fg_color, "Enter two valid UUIDs");
                    }
                }
                PaletteDialog::Click {
                    offset,
                    ...
                    byte_offset_field(ui, "Offset", offset, len);
                    enum_combo(ui, "dlg-stop", stop, &CURSOR_STOPS);
                    ui.checkbox(select, "Select");
                    ui.checkbox(extend, "Extend selection");
                    ui.checkbox(select_syntax_node, "Select syntax node");
                }
                PaletteDialog::Drag { offset } => {
                    byte_offset_field(ui, "Offset", offset, len);
                }
                PaletteDialog::DragSelectionHandle { fixed, position } => {
                    byte_offset_field(ui, "Fixed offset", fixed, len);
                    byte_offset_field(ui, "Handle offset", position, len);
                }
                PaletteDialog::ReplaceWholeFile { text } => {
                    ui.colored_label(ui.visuals().warn_fg_color, "Replaces the entire document");
...
impl BlockEditor for TextEditor {
    ...
    fn direct_editor_ui(
        ...
        self.command_palette_dialog(ui.ctx());
