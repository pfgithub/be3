...
pub fn listbox(document: &mut Document, labels: &[&str], selected: Option<usize>) -> NodeId {
    choice::choice(document, labels, selected, Kind::Listbox)
}
...
pub fn set_listbox_selected(document: &mut Document, control: NodeId, selected: Option<usize>) {
    choice::set_selected(document, control, selected);
}

pub fn set_listbox_on_change(
