...
#[tokio::test]
async fn logic_game_solution_reference_is_excluded_and_resolves_to_the_target() {
    ...
    let referencing = client.create_block(LogicGame::new());
    let target = client.create_block(LogicGrid::new());
    data.loaded().await;
    ...
    referencing.operate(LogicGameOperation::InsertSolution {
