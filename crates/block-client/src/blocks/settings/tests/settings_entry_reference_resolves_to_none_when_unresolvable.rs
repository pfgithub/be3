...
#[tokio::test]
async fn settings_entry_reference_resolves_to_none_when_unresolvable() {
    ...
    let referencing = client.create_block(Settings::new());
    data.loaded().await;
    ...
    referencing.operate(SettingsOperation::SetEntry {
        block_type,
        activation: ActivationCondition::Fallback,
        block: broken_reference,
    });
    client.synchronized().await;
