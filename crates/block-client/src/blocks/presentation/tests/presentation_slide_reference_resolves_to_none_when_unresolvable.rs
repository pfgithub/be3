...
#[tokio::test]
async fn presentation_slide_reference_resolves_to_none_when_unresolvable() {
    ...
    let referencing = client.create_block(Presentation::new());
    data.loaded().await;
    ...
    referencing.operate(PresentationOperation::Insert {
        slide: PresentationSlide {
            id: Uuid::new_v4(),
            block_id: broken_reference,
        },
        index: 0,
    });
    client.synchronized().await;
