...
#[tokio::test]
async fn video_clip_reference_resolves_to_none_when_unresolvable() {
    ...
    let referencing = client.create_block(Video::new());
    data.loaded().await;
    ...
    let clip = VideoClip::new(broken_reference, 10);
    referencing.operate(VideoOperation::InsertClip { clip, index: 0 });
    client.synchronized().await;
