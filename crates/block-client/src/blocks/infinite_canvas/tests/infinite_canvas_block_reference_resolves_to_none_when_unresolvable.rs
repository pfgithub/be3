...
#[tokio::test]
async fn infinite_canvas_block_reference_resolves_to_none_when_unresolvable() {
    ...
    let referencing = client.create_block(InfiniteCanvas::new());
    data.loaded().await;
    ...
    let entity = block_entity(Uuid::new_v4(), broken_reference);
    referencing.operate(InfiniteCanvasOperation::Add { entity });
    client.synchronized().await;
