...
#[tokio::test]
async fn hotbar_component_reference_resolves_to_none_when_unresolvable() {
    ...
    let referencing = client.create_block(Hotbar::new());
    data.loaded().await;
    ...
    referencing.operate(HotbarOperation::SetSlots {
        slots: vec![HotbarSlot::Component {
            name: "Pinned".to_owned(),
            compiled: broken_reference,
        }],
    });
    client.synchronized().await;
