...
#[tokio::test]
async fn database_schema_reference_is_excluded_and_resolves_to_the_target() {
    ...
    let referencing = client.create_block(Database::new(BlockRef::Direct(data.id())));
    let target = client.create_block(DatabaseSchema::new());
    data.loaded().await;
    ...
    let value = Database::new(reference);
