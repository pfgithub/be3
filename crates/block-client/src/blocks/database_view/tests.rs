...
mod database_view_reference_is_excluded_and_resolves_to_the_target;
mod database_view_reference_resolves_to_none_when_unresolvable;
...
impl TestServer {
    async fn spawn() -> Self {
        let root = std::env::temp_dir().join(format!(
            "block-client-database-view-test-{}",
            Uuid::new_v4()
        ));
...
async fn identity(url: &str) -> (Uuid, String, Uuid) {
    ...
    let session = management
        .register(
            ...
            "Test",
            "database-view-block-test-password",
        )
        .await
        .unwrap();
