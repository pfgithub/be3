...
fn touch_input_draws_notes_stroke() {
    ...
    assert_eq!(
        ui.notes
            .coverage_at(size, Vector::new(120.0, 160.0), &atlas(&mut pixels)),
        255
    );
    assert_eq!(
        ui.notes
            .coverage_at(size, Vector::new(150.0, 175.0), &atlas(&mut pixels)),
        255
    );
    assert_eq!(
        ui.notes
            .coverage_at(size, Vector::new(180.0, 190.0), &atlas(&mut pixels)),
        255
    );
}
