...
Only where the host gives it a viewport of its own, though. Embedded in something that pans and zooms already - a block being edited inside an infinite canvas - the host takes pan and zoom back, turning the gestures over the region into commands for the canvas, and tells the instance so: host.owns_pan_and_zoom() answers false, and the editor then leaves its view at the identity, fitting its content to whatever region it is given, and hides the controls that would steer it. An editor embedded in a host that does not pan and zoom itself - a block inside a text block - keeps its own view, since nothing else would move it.
