...
- [x] Make embedded presentation slides fill the playback area with containing black bars and auto-hiding overlay controls.
- [x] Persist a configurable infinite-canvas preview region and expose its position and size in the inspector.
- [x] Render the preview-region outline while editing and use it for preview/sub-editor sizing and clipping; keep root canvas interaction unbounded.
- [x] When an infinite canvas has a preview region, allow nested resizing and propagate the new size into that region.
